...
const LogResultContainer = () => {
  ...
  const relatedPeople = MOCK_LOGS.filter(log => String(log.id) !== String(id));

  // Lifting up like state
  const [liked, setLiked] = useState(false);
  const [likeCount, setLikeCount] = useState(selectedLog.likes);
  ...
  // Scroll to top, sync like state, and reset carousel index when log ID changes
  useEffect(() => {
    window.scrollTo(0, 0);
    setLiked(false);
    setLikeCount(selectedLog.likes);
    setCurrentIndex(relatedPeople.length);
  }, [id, selectedLog.likes, relatedPeople.length]);
            ...
            <S.SliderWindow>
              ...
              >
                {extendedPeople.map((person, idx) => (
                  <S.RelatedCard key={`${person.id}-${idx}`} onClick={() => {
                    const currentTab = location.pathname.includes('/report/patterns')
                      ? 'report/patterns'
                      : location.pathname.includes('/report/action-plan')
                        ? 'report/action-plan'
                        : 'detail';
                    navigate(`/logs/result/${person.id}/${currentTab}`);
                  }}>
